...
public:
  ...
  String login() {
    ...
    String body = "username=" + urlEncode(_user) + "&response=" + urlEncode(response);
  ...
  String aha(const String& sid, const String& switchcmd,
    ...
    String url = _base + "/webservices/homeautoswitch.lua?switchcmd=" + urlEncode(switchcmd) +
                 "&sid=" + urlEncode(sid);
...
private:
  ...
  static String normalize_ain(const String& ain) {
    String out; out.reserve(ain.length());
    for (size_t i = 0; i < ain.length(); ++i) if (ain[i] != ' ') out += ain[i];
    return out;
  }
  ...
  static String md5_legacy_response(const String& challenge, const String& password) {
    String comb = challenge + "-" + password;
    std::vector<uint8_t> u16; u16.reserve(comb.length()*2);
    for (size_t i = 0; i < comb.length(); ++i) {
      uint16_t cp = (uint8_t)comb[i];
      if (cp > 255) cp = '.';
      u16.push_back((uint8_t)cp); u16.push_back(0x00);
    }
  ...
  static String pbkdf2_response(const String& challenge, const String& password) {
    ...
    auto hex2bin = [](const String& hx){
      std::vector<uint8_t> v; v.reserve(hx.length()/2);
    ...
    mbedtls_pkcs5_pbkdf2_hmac_ext(MBEDTLS_MD_SHA256,
      h1, 32, s2.data(), s2.size(), iter2, 32, h2);
  ...
  static String toHex(const uint8_t* buf, size_t n) {
    ...
    for (size_t i=0;i<n;i++){ s += h[buf[i]>>4]; s += h[buf[i]&15]; }
    return s;
  }
};
