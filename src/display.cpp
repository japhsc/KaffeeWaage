...
int Display::mapDigit(int d) const { return DISPLAY_RIGHT_TO_LEFT ? d : (7 - d); }

void Display::putChar(int pos, char c, bool dp){ lc_.setChar(0, mapDigit(pos), c, dp); }
void Display::putDigit(int pos, uint8_t d, bool dp){ lc_.setChar(0, mapDigit(pos), char('0'+(d%10)), dp); }

void Display::clear(){ for(int i=0;i<8;i++) putChar(i, ' '); }
...
void Display::renderNumberMg(int32_t mg){
  ...
  // tenths
  putDigit(pos++, d10 % 10, false); d10 /= 10;
  // ones with decimal point
  putDigit(pos, d10 % 10, true); pos++; d10 /= 10;
  // higher digits
  while(d10 > 0 && pos < 8){ putDigit(pos++, d10 % 10, false); d10 /= 10; }
...
void Display::showError(){ clear(); putChar(7,'E'); putChar(6,'r'); putChar(5,'r'); }
