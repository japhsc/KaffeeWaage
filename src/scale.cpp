...
void Scale::update(){
  ...
  if(!hx_.is_ready()){ ok_ = false; return; }
  ok_ = true;
  ...
  buf_[bi_] = weight_raw_; bi_ = (bi_+1)%3;
  ...
  if (wcount_ == STAB_WINDOW_SAMPLES) {
    ...
    int64_t var = ex2 - (int64_t)mean * mean; if (var < 0) var = 0;
    int32_t stdmg = (int32_t)sqrt((double)var);
    int32_t p2p = mx - mn;

    bool quiet = (stdmg <= STAB_STDDEV_MG) && (p2p <= STAB_P2P_MG);
    if (quiet) {
      // BUGFIX: only set stable_since_ when entering quiet state, not every tick
